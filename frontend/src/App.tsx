...
const services: Service[] = [
  { name: 'n8n', category: 'Automatisation', icon: Workflow, description: 'Workflows, newsletters et tâches du quotidien.', next: 'Créer ton compte propriétaire dans n8n, puis construire le premier workflow.', statusKey: 'n8n', url: 'http://127.0.0.1:5678/' },
...
export default function App() {
  ...
  return <div className="shell">
    ...
    <div className="workspace"><header className="topbar"><span>Homelab <ChevronRight size={14}/> <strong>{page}</strong></span><span className="local-pill">Accès local</span></header>
      <main id="main">
        ...
        {page === 'Paramètres' && <section className="panel"><h2>Une installation personnelle</h2><dl><div><dt>Version</dt><dd>{status?.version ?? '0.1.0'}</dd></div><div><dt>Adresse</dt><dd>localhost:3000</dd></div><div><dt>Accès distant</dt><dd>Non activé</dd></div><div><dt>Authentification</dt><dd>À mettre en place avant l’accès réseau</dd></div></dl><p className="muted">Cette première version est accessible uniquement depuis cet ordinateur. La prochaine étape est l’installation de n8n et de sa base de données.</p></section>}
        {page === 'Vue d’ensemble' && <section className="next-step"><div className="step-number">01</div><div><span className="eyebrow">LA SUITE DU HOMELAB</span><h2>Place aux automatisations.</h2><p>n8n et PostgreSQL seront les premiers services à rejoindre ton espace.</p></div><button className="button" onClick={() => setSelected(services[0])}>Voir la prochaine étape <ChevronRight size={16}/></button></section>}
