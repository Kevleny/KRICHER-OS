  ...
  return <div className="shell">
    ...
    <div className="workspace"><header className="topbar"><span>Homelab <ChevronRight size={14}/> <strong>{page}</strong></span><span className="local-pill">Accès sécurisé</span></header>
      <main id="main">
        ...
        {page === 'Vue d’ensemble' && <section className="home-tools"><div className="home-tools-head"><div><p className="eyebrow">SURVEILLANCE ET ASSISTANT</p><h2>Le serveur veille sur ses services.</h2><p className="muted">Contrôle automatique toutes les deux minutes · confirmation avant chaque action manuelle</p></div><button className="button assistant-button" onClick={() => setPage('Assistant')}><Bot size={17}/> Ouvrir le chat</button></div><div className="health-pills">{Object.entries(watchedLabels).map(([key, label]) => <span key={key}><i className={`dot ${status?.watchdog?.services?.[key]?.healthy ? '' : 'offline'}`}/>{label}</span>)}</div><div className="home-actions"><button disabled={controlBusy} className="button subtle" onClick={() => proposeTarget('n8n')}><RotateCcw size={15}/> Redémarrer n8n</button><button disabled={controlBusy} className="button subtle" onClick={() => proposeTarget('stack')}><RotateCcw size={15}/> Tous les services</button><button disabled={controlBusy} className="button danger" onClick={() => proposeTarget('host')}><Power size={15}/> Windows</button><button className="button subtle" onClick={() => setPage('Infrastructure')}>Voir tous les contrôles <ChevronRight size={15}/></button></div>{controlMessage && <p className="action-message" role="status">{controlMessage}</p>}</section>}
        ...
        {page === 'Vue d’ensemble' && <section className="next-step"><div className="step-number"><ShieldCheck size={28}/></div><div><span className="eyebrow">SURVEILLANCE AUTOMATIQUE</span><h2>Réparation avec garde-fous.</h2><p>Windows contrôle les cinq services. n8n vérifie régulièrement leur état depuis l’intérieur.</p></div><button className="button" onClick={() => setPage('Assistant')}>Parler à l’assistant <ChevronRight size={16}/></button></section>}
